...
$cronController = new CronController('mbbank2');
if ($_GET['type'] == $tkuma->site('pin_cron')) {
	...
	$getlist_momo = $tkuma->get_list("SELECT * FROM `account_mbbank2` WHERE `status` = ? ORDER BY `id` ASC ", ['success']);

	if ($getlist_momo) {
		#1
		foreach ($getlist_momo as $rows) {
			$mbbank = new MBB2($rows['corpid'], $rows['phone'], $rows['password'], $rows['stk']);

			$gethistt = $mbbank->getTransactionHistoryV2($rows['stk']);
			...
			if ($gethistt->data->transactionHistoryList != null || $gethistt->data->transactionHistoryList != "") {
				foreach ($gethistt->data->transactionHistoryList as $ROWHIST) {
				//	sendMessTelegramNew("Lịch sử giao dịch: " . json_encode($ROWHIST));
					...
					$gettranid = $ROWHIST->transactionRefNo;
				...
				//	sendMessTelegramNew("gettranid " . $gettranid);
	              ...
	              //  sendMessTelegramNew("TranIdd " . $tranIdd);
	                
	                $isMbbank = $ROWHIST->bankName == "MB"; 
					
				//	sendMessTelegramNew("isMbbank:  " . $isMbbank);
					...
					if (!$getuser) {
						...
						$getuser = $tkuma->get_row("SELECT * FROM `users` WHERE `stk` = ? ", [$ROWHIST->reciprocalAcct]);
				...
				//	}
				  ...
				  //  sendMessTelegramNew("getuser :" . $getuser['stk']);
					...
					$dataline = "MBBANK2|" . date("d/m/Y") . "|" . $ROWHIST->postingDate . "|+" . format_cash($amount) . "|" . $gettranid . "|" . $settings1 . "|" . getRowRealtime2('account_mbbank2', 'phone', $phonenhan, 'name') . "|" . $phonenhan;
					$tranIdd2 = 0;
					...
					if ($gettranIdc || $ROWHIST->debitAmount != 0) {
					  //  sendMessTelegramNew("đã tồn tại mã gd");
						...
						if ($tkuma->site('status_bipmbbank') == '1' && !$isMbbank ) {
							if (strpos($settings1, 'tu:') === false) {
								$tranIdd2 = $tranIdd;
								$tranIdd = chonglo($tranIdd, $comment);
								$dataline = "MBBANK2|" . date("d/m/Y") . "|" . $ROWHIST->postingDate . "|+" . format_cash($amount) . "|" . substr($gettranid, 0, 2) . $tranIdd . $dauky . "|" . $settings1 . "|" . getRowRealtime2('account_mbbank2', 'phone', $phonenhan, 'name') . "|" . $phonenhan;
							}
						}
